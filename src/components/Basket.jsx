...
class Basket extends React.Component {
    ...
    render() {
        ...
        let basketTable = <table className="table align-middle text-center">
            <thead>
            <tr>
                <th scope="col"></th>
                <th scope="col"></th>
            ...
            {this.state.basketItems.map((item, index) =>
                <tr className="table-row" key={item.db_id}>
                    <td>
                    ...
                    <td>
                        <Counter count={item.count} bd_id={item.db_id} index={index} updateQuantityCallback={this.updateQuantity}/>
                    </td>
                    <td>
                        <div className="position-relative">
                            {item.count * item.price}
                            <div className="basket__delete-icon">
                                <button type="button" className="btn btn-outline-dark" onClick={() => this.deleteBasketItem(index)}>
                ...
                <div className="container-md modal-container basket-container bg-white">
                    <div className="container pb-3">
                        ...
                        <div className="text-end">
                            ...
                            <button className="btn btn-dark" onClick={this.getResponse}>Оформити замовлення</button>
                        </div>
                    </div>
                </div>
            </div>;
